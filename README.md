...
- `コース料理` は `おつまみコース` として扱います。
- `おつまみコース`、`オコノミコース` は基本セット品として `広ナ`、`ママカリ`、`クラゲ`、`ナス`、`ブタシソ` をコース内販売点数に加算します。
- `忘年会コース`、`新年会コース`、`歓送迎会コース` は `季節宴会コース` として扱い、`クラゲ` の代わりに `牡蠣オイル` を加算します。
- 3名以上時の `がんす`、`鉄MIX` は、月次集計データだけでは対象判定ができないため自動加算していません。
- `【コース】` で始まる商品はコース内訳商品として扱い、通常の商品売上TOP、ABC、増減ランキングからは除外します。
- `【ランチ】`、`【ランチTO】`、`【TOランチ】` で始まる商品はランチ分析で別集計します。通常の商品売上TOP、ABC、増減ランキングからは除外します。
- ランチ時間帯に販売された通常商品やドリンクは、商品名だけでは判定できないためランチ分析には含めていません。
- `アスパラ` は期間限定確認、`ボトル` と `アイス` は神田店のみ販売として注意表示します。
